...
import { TimelineElement, TimelineTrack } from "@/lib/opencut/types/timeline";
import { useMediaStore } from "@/lib/opencut/stores/media-store";
import { MediaFile } from "@/lib/opencut/types/media";
import { usePlaybackStore } from "@/lib/opencut/stores/playback-store";
import { useEditorStore } from "@/lib/opencut/stores/editor-store";
import { Button } from "@/components/ui/button";
import { Play, Pause, Expand, SkipBack, SkipForward } from "lucide-react";
import { useState, useRef, useEffect, useCallback } from "react";
import { renderTimelineFrame } from "@/lib/opencut/lib/timeline-renderer";
import { cn } from "@/lib/opencut/lib/utils";
import { formatTimeCode } from "@/lib/opencut/lib/time";
import { EditableTimecode } from "@/components/ui/editable-timecode";
import { useFrameCache } from "@/lib/opencut/hooks/use-frame-cache";
import { useSceneStore } from "@/lib/opencut/stores/scene-store";
import {
  DEFAULT_CANVAS_SIZE,
  DEFAULT_FPS,
  useProjectStore,
} from "@/lib/opencut/stores/project-store";
import { TextElementDragState } from "@/lib/opencut/types/editor";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
...
import { PLATFORM_LAYOUTS, type PlatformLayout } from "@/lib/opencut/stores/editor-store";
...
export function PreviewPanel() {
  ...
  useEffect(() => {
    const draw = async () => {
          ...
          preRenderNearbyFrames(
            ...
            async (time: number) => {
              ...
              try {
                ...
                // If CORS error occurs, throw to skip caching
      ...
      // Try to cache the frame, but skip if CORS error occurs
      try {
        ...
        // If CORS error occurs (canvas tainted), skip caching but still render
        const errorMessage = (error as Error)?.message || String(error);
        if (
          error instanceof DOMException && error.name === 'SecurityError' ||
          errorMessage.includes('tainted') ||
          ...
          console.warn('Skipping frame cache due to CORS restriction (canvas tainted by cross-origin video)');
        } else {
