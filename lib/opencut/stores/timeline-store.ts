...
} from "@/lib/opencut/types/timeline";
import { useMediaStore, getMediaAspectRatio } from "./media-store";
import { MediaFile, MediaType } from "@/lib/opencut/types/media";
import { findBestCanvasPreset } from "@/lib/opencut/lib/editor-utils";
import { storageService } from "@/lib/opencut/lib/storage/storage-service";
import { useProjectStore } from "./project-store";
import { useSceneStore } from "./scene-store";
import { generateUUID } from "@/lib/opencut/lib/utils";
import { TIMELINE_CONSTANTS } from "@/lib/opencut/constants/timeline-constants";
import { checkElementOverlaps, resolveElementOverlaps } from "@/lib/opencut/lib/timeline";
...
interface TimelineStore {
  ...
  // Internal method to load tracks directly (for integration purposes)
...
export const useTimelineStore = create<TimelineStore>((set, get) => {
  ...
  return {
    ...
    addElementToTrack: (trackId, elementData) => {
      console.log(`📌 addElementToTrack called:`, {
        trackId,
        elementType: elementData.type,
        elementName: elementData.name,
        mediaId: elementData.type === "media" ? elementData.mediaId : undefined,
      ...
      if (!track) {
        console.error("❌ Track not found:", trackId, "Available tracks:", get()._tracks.map(t => ({ id: t.id, type: t.type })));
        return;
      }
      ...
      console.log(`💾 Updating tracks:`, {
        ...
        trackElementsBefore: track.elements.length,
        trackElementsAfter: updatedTracks.find(t => t.id === trackId)?.elements.length || 0,
      ...
      console.log(`✅ Element added successfully. Current tracks:`, get().tracks.map(t => ({
        id: t.id,
        type: t.type,
    ...
    addElementAtTime: (item: MediaFile | TextElement, currentTime = 0) => {
      console.log(`🎬 addElementAtTime called:`, {
        itemType: item.type,
        itemName: item.name,
        currentTime,
        itemId: item.id,
      ...
      if (item.type === "text") {
        ...
        const element = buildTextElement(item, currentTime);
        console.log(`📝 Adding text element to track ${targetTrackId}:`, element);
        get().addElementToTrack(targetTrackId, element);
        console.log(`✅ Text element added. Total tracks: ${get().tracks.length}`);
        return true;
      }
      ...
      console.log(`🎥 Adding media element to track ${targetTrackId}:`, {
        element,
        mediaUrl: media.url?.substring(0, 50),
        mediaDuration: media.duration,
      });
      get().addElementToTrack(targetTrackId, element);
      console.log(`✅ Media element added. Total tracks: ${get().tracks.length}, Track elements: ${get().tracks.find(t => t.id === targetTrackId)?.elements.length || 0}`);
      return true;
    },
    ...
    // Internal method to load tracks directly (for integration purposes)
    _loadTracks: (tracks: TimelineTrack[]) => {
      updateTracks(tracks);
    },
  };
});
